...
def get_cloud_run_logs():
    ...
    for metric_config in config['metrics_cloudrun']:
        # metrics from config
        ...
        cloud_run_logs = client.list_time_series(
            name=project_name,
            filter=(f'metric.type="{metric_type}"'),
            interval=interval,
            view=monitoring_v3.ListTimeSeriesRequest.TimeSeriesView.FULL,
        )
        print("logsssssssssss:", cloud_run_logs)
    #     for ts in cloud_run_logs:
    ...
    #             'zone': ts.resource.labels['location'],
    #             'service_name': ts.resource.labels['service_name'],
    ...
    #                 "description": f"The instance have exceeded the threshold by value:{instance_data['value']} for {instance_data['subcategory_name']} in {instance_data['service_name']}",
...
cloud_run_logs = PythonOperator(
    task_id='get_cloud_run_metrics',
    python_callable=get_cloud_run_logs,
    dag=dag
)
